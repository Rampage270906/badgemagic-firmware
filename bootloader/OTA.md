...
**[IMAGE 1 — middle portion: the subfolder contents with `wch-ota-tool.exe`]**
...
**Always flash the binary matching the *inactive* slot.** Sending the wrong slot's binary can produce corrupted, unpredictable firmware behavior — see PR discussion for details. If Slot A is active, send the Slot-B-linked `.bin`; if Slot B is active, send the Slot-A-linked `.bin`.
